...
<head>
    ...
    <style>
        ...
        .logo-icv { max-width: 120px; height: auto; margin-bottom: 5px; }

        .profile-container { position: relative; width: 85px; height: 85px; margin: 10px auto 10px; }
        ...
        .brand-name { font-weight: 800; font-size: 0.75rem; letter-spacing: 1px; margin-bottom: 20px; text-transform: uppercase; color: var(--primary); margin-top: 5px; }
...
<div class="card">
    <div style="display: flex; justify-content: space-between; align-items: center;">
        <img src="logo.png" alt="ICV Logo" class="logo-icv" onerror="this.style.display='none'">
        <button onclick="document.body.classList.toggle('dark-mode')" style="background:none; border:none; cursor:pointer; font-size: 1.3rem;">🌓</button>
    </div>
    ...
    <div class="brand-name">Gestión de Turnos 10x10</div>
    ...
    <div class="config-section">
        ...
        <input type="text" id="nombre" placeholder="Tu nombre aquí" oninput="saveData()">
    </div>

    <div style="text-align: left;">
        <label>Consultar Fecha</label>
        <input type="date" id="fechaBusqueda" onchange="calcular()">
    </div>

    <div id="contadorBajada" class="widget">Calculando turno...</div>
    <div id="resultado" class="result-box"></div>

    <label>Ciclo de 10 días:</label>
    <div class="mini-calendar" id="miniCalendar"></div>

    <button class="btn btn-pdf" onclick="descargarPDF()">📄 Exportar PDF</button>
    <button class="btn btn-ws" onclick="compartirWhatsApp()">💬 Enviar por WhatsApp</button>

    <div class="footer">
        <strong>Mi instagram @Otherbabu.u </strong> | Faena ICV-DMH OPERACIONES
    </div>
</div>

<script>
    // Lógica automática basada en tus ciclos reales
    const DATA_TURNOS = {
        "1": { inicio: "2026-01-07", tipo: "DIA", ciclo: 40 },
        "2": { inicio: "2026-01-07", tipo: "NOCHE", ciclo: 40 },
        "3": { inicio: "2025-12-28", tipo: "DIA", ciclo: 40 },
        "4": { inicio: "2025-12-28", tipo: "NOCHE", ciclo: 40 },
        "5": { inicio: "2025-12-23", tipo: "DIA", ciclo: 20 },
        "6": { inicio: "2026-01-02", tipo: "DIA", ciclo: 20 },
        "7": { inicio: "2026-01-12", tipo: "DIA", ciclo: 40 },
        "8": { inicio: "2026-01-12", tipo: "NOCHE", ciclo: 40 }
    };
    ...
    function calcular() {
        const gKey = document.getElementById('grupo').value;
        const config = DATA_TURNOS[gKey];
        const fBusca = document.getElementById('fechaBusqueda').value;
        if (!fBusca) return;

        const inicio = new Date(config.inicio + "T12:00:00");
        const busca = new Date(fBusca + "T12:00:00");
        const diff = Math.floor((busca - inicio) / (1000*60*60*24));
        
        const cicloTotal = config.ciclo;
        ...
        if (cicloTotal === 20) {
            if (pos < 10) {
                actualizarUI("TURNO DÍA ☀️", "#fff3e0", "#ef6c00", "#ff9800");
                cont.innerText = `Bajas en: ${10 - pos} días`;
            } else {
                actualizarUI("DÍA LIBRE 🏠", "#e8f5e9", "#2e7d32", "#4caf50");
                cont.innerText = `Subes en: ${20 - pos} días`;
            }
        } else {
            if (pos < 10) {
                actualizarUI("TURNO DÍA ☀️", "#fff3e0", "#ef6c00", "#ff9800");
                ...
                actualizarUI("DÍA LIBRE 🏠", "#e8f5e9", "#2e7d32", "#4caf50");
                cont.innerText = `Subes a NOCHE en: ${20 - pos} días`;
            } else if (pos < 30) {
                actualizarUI("TURNO NOCHE 🌙", "#e8eaf6", "#283593", "#3f51b5");
                cont.innerText = `Bajas en: ${30 - pos} días`;
            } else {
                actualizarUI("DÍA LIBRE 🏠", "#e8f5e9", "#2e7d32", "#4caf50");
                cont.innerText = `Subes a DÍA en: ${40 - pos} días`;
            }
        }
    ...
    function descargarPDF() {
        ...
        const doc = new jsPDF();
        doc.text("CONSULTA TURNO ICV", 20, 20);
        doc.text("Nombre: " + document.getElementById('nombre').value, 20, 35);
        doc.text("Grupo: " + document.getElementById('grupo').value, 20, 45);
        doc.text("Estado: " + document.getElementById('resultado').innerText, 20, 60);
        doc.save("Turno_ICV.pdf");
    }

    function compartirWhatsApp() {
        const msj = encodeURIComponent(`*CONSULTA TURNO ICV*\n👤 *${document.getElementById('nombre').value}*\n📍 Estado: *${document.getElementById('resultado').innerText}*`);
